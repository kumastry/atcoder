...
int sum_node(node_type *p) {
    ...
    if(p->left != NULL && p->right != NULL) {
        return sum_node(p->left) + sum_node(p->right) + p->key;
    } else if(p->right != NULL) {
        return sum_node(p->right) + p->key;
    } else {
        return sum_node(p->left) + p->key;
    }
}
